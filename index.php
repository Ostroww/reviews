...
if (!empty($_POST)) {
    ...
    if (empty($erreurs)) {
        $query = $db->prepare('INSERT INTO reviews (name, review, note, created_at) VALUES (:name, :review, :note, :created_at)');
        $query->execute([
            'name' => $name, 'review' => $comment, 'note' => $notation, 'created_at' => $created_at,
        ]);
...
<body>
    <div class="container">
        <table class="table">
            <tr><td>Notre moyenne<td><td></td></tr>
            <tr>
                ...
                <td align=center>
                    <table>
                        ...
                        <div class="note">
                            ...
                            echo $i; ?>
                            <i class="fa-solid fa-star gold"></i>
                            <!--<progress value="<?php echo $numb[$i] ?>" max="<?php echo $nb ?>">
                            </progress>-->
                            <div class="progress">
...
<?php if ($success) { ?>
    ...
    <div>
        <table class="table">
            <tr><td colspan="2"><?php echo $name?></td><td></td></tr>
            <tr><td id="comment" colspan="2"><?php 
                $black = (5-$notation);
                ...
                for ($z = 0; $z < $black; $z++) { ?>
                    ...
                    <?php } ?>
            <?php echo '<br>'.$comment?></td><td></td></tr>
                <tr><td id="jour"><?php echo dateToFrench($created_at, "l j F Y")?></td></tr>
            </table>
        </div>
<?php } ?>
...
<?php if (!$success) { ?>
    ...
    <div class="container">
        <table class="table">
            <tr><td>Publier un avis</td></tr>
        
            <?php if (!empty($erreurs)) { ?>
                <tr><td><ul class="alert alert-danger">
    ...
    foreach ($reviews as $review) { 
        ...
        str_split($first, 1); ?>
            <div class="container">
                ...
                <div>
                    <table class="table">
                        <tr><td colspan="2"><?php echo $review['name']?></td><td></td></tr>
                        <tr><td id="comment" colspan="2"><?php 
                        $black = (5-$review['note']);
                        ...
                        <?php }
                            for ($z = 0; $z < $black; $z++) { ?>
                                <i class="fa-solid fa-star"></i>
                            <?php }
                        ?><?php echo '<br>'.$review['review']?></td><td></td></tr>
                            <tr><td id="jour"><?php echo dateToFrench($review['created_at'], "l j F Y")?></td></tr>
                        </table>
                    </div>
                </div>
                <?php } ?>
